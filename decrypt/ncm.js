...
module.exports.Decrypt = async (file, raw_filename, raw_ext) => {
  // const fileBuffer = await GetArrayBuffer(file)
  const fileBuffer = toArrayBuffer(file)
  ...
  try {
    if (musicMeta.format === "mp3") {
      audioData = await WriteMp3Meta(audioData, artists, info.title, musicMeta.album, imageInfo.buffer, musicMeta.albumPic)
